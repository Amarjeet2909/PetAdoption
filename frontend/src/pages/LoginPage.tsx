            ...
            <div className="flex items-center justify-center bg-gray-50 px-4 sm:px-6 py-8 sm:py-12">
                <div className="w-full max-w-md animate-slideUp">
                    <div className="text-center mb-8">
                        ...
                        <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 mt-3">Welcome Back!</h1>
                        <p className="text-gray-500 mt-1 text-sm">Login to continue finding your perfect companion</p>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 space-y-5">
                        ...
                        <div>
                            ...
                            <input type="password" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={handleKeyDown} />
                        </div>
                        <button onClick={handleLogin} disabled={loading} className="w-full bg-orange-500 hover:bg-orange-600 disabled:opacity-60 text-white font-semibold py-3 rounded-xl transition-all hover:scale-[1.02] shadow-md">{loading ? "Logging in... 🐾" : "Login →"}</button>
                    </div>
                    <p className="text-center text-sm text-gray-500 mt-6">Don't have an account? <Link to="/register" className="text-orange-500 font-semibold hover:underline">Register here 🐶</Link></p>
                </div>
            </div>
        </div>
    );
